...
static bool perform_refresh(const char *wake_label, int64_t boot_time_us)
{
    ...
    img = download_image(&sleep_seconds, &server_epoch, &http_status,
                         ...
                         (button_wake || ota_is_pending_verify()) ? NULL : last_content_id,
                         new_content_id);
    ...
    /* Calibration headers accompany healthy 204 and 200 responses. Adopt a
     * well-backed seed before the no-image branch so skip-refresh cycles do not
     * starve a freshly reset calibration. */
    if ((img || http_status == 204) && cal_seed_n >= 0 &&
        scheduler_adopt_cal_seed(cal_seed_ppm, cal_seed_n)) {
